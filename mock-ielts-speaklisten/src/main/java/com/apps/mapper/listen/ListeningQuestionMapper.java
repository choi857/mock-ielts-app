...
public interface ListeningQuestionMapper {
    @Insert("INSERT INTO COL_LISTENING_QUESTION (COL_ID,COL_LISTENING_ID, COL_TYPE, COL_CONTENT, COL_PLACEHOLDER_FORMAT, COL_PART, COL_CREATED_AT, COL_UPDATED_AT) VALUES (#{id},#{listeningId}, #{type}, #{content}, #{placeholderFormat}, #{part}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)")
    ...
    @Update("UPDATE COL_LISTENING_QUESTION SET COL_TYPE = #{type}, COL_CONTENT = #{content}, COL_PLACEHOLDER_FORMAT = #{placeholderFormat}, COL_PART = #{part}, COL_UPDATED_AT = CURRENT_TIMESTAMP WHERE COL_ID = #{id}")
