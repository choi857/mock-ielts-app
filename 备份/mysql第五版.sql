...
#     ADD COLUMN COL_PART ENUM('Part1', 'Part2', 'Part3', 'Part4') COMMENT '题目所属部分' AFTER COL_UPDATED_AT;# 第五版开始
...
CREATE TABLE COL_SPEAK_USER_ANSWER_DETAIL (
                                              ...
                                              USER_AUDIO_URL_TO_AI TEXT COMMENT '用户回答的音频URL（提供给AI转录）',
                                              CREATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '提交时间',
                                              COL_PART ENUM('Part1', 'Part2', 'Part3') COMMENT '题目所属部分'
) COMMENT='用户口语答案明细表（通过RECORD_ID实现批量关联）';
CREATE TABLE COL_PAPER (
         PAPER_ID BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '试卷ID',
         PAPER_NAME VARCHAR(255)  COMMENT '试卷名称',
         PAPER_DESCRIPTION TEXT COMMENT '试卷描述',
         READING_SUMMARY_ID BIGINT  COMMENT '关联的阅读题ID',
         LISTEN_ID BIGINT  COMMENT '关联的听力题ID',
         WRITE_ID BIGINT  COMMENT '关联的写作题ID',
         SPEAK_ID BIGINT  COMMENT '关联的口语题ID',
         CREATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
         UPDATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'
) COMMENT='试卷表';
